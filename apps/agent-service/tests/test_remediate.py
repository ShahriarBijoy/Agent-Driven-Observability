...
async def test_gate_approved_with_server_marker_but_no_stash_is_replay_denied(approvals):
    ...
    approvals["apr-1"] = {
        "run_id": "run-1", "decision": "approved",
        "summary": f"scale gateway{remediate.server_verified_marker(aid, 'deadnonce')}",
    }
...
async def test_replay_of_approval_after_identical_re_dry_run_is_rejected(fake_kubectl, approvals):
    """A fresh dry-run with IDENTICAL params (same action_id, since the
    fingerprint doesn't cover the nonce) must re-arm the stash with a NEW
    nonce, so an approval minted against the FIRST dry-run's marker can no
    longer authorize execution — one approval must not cover repeated
    identical dry-run/execute rounds."""
    ctx = _FakeCtx()
    first_dry = await remediate.scale_deployment(ctx, "gateway", replicas=4, dry_run=True)
    aid = first_dry["action_id"]
    stale_block = remediate.server_verified_block(ctx.run_id, aid)
    approvals["apr-1"] = {"run_id": "run-1", "decision": "approved",
                           "summary": f"scale gateway{stale_block}"}

    # Re-dry-run the identical action: same action_id, fresh nonce.
    second_dry = await remediate.scale_deployment(ctx, "gateway", replicas=4, dry_run=True)
    assert second_dry["action_id"] == aid

    result = await remediate.scale_deployment(
        ctx, "gateway", replicas=4, dry_run=False, approval_id="apr-1"
    )
    assert "error" in result
    # The stale approval's marker doesn't match the current (re-armed) entry.
...
async def test_update_db_secret_url_encodes_password_with_reserved_chars(
    fake_kubectl_secret, approvals
):
    """A rotated password containing URL-reserved characters (`@`, `:`, `/`)
    must be percent-encoded when built into DATABASE_URL — otherwise those
    characters would be parsed as URL delimiters instead of part of the
    password, corrupting the connection string."""
    calls, _old_password, _new_password = fake_kubectl_secret
    weird_password = "p@ss:w/ord#1"
    with open(remediate.config.vault_file, "w", encoding="utf-8") as fh:
        fh.write(weird_password)
    ...
    patch_call = calls[-1]
    body = json.loads(patch_call[patch_call.index("-p") + 1])
    assert base64.b64decode(body["data"]["POSTGRES_PASSWORD"]).decode() == weird_password
    database_url = base64.b64decode(body["data"]["DATABASE_URL"]).decode()
    import urllib.parse

    assert database_url == (
        f"postgres://lab:{urllib.parse.quote(weird_password, safe='')}@postgres:5432/observability_lab"
    )
    # The raw (unencoded) password must never appear verbatim inside the URL.
    assert weird_password not in database_url
...
# ---- end-to-end: dry-run -> request_approval -> execute, driven ONLY by ----
# real tool results (final-review CRITICAL fix: RunContext.request_approval
# must hand back the approval_id the execute gate requires by name, and
# sdk.py's request_approval tool must surface it) --------------------------


async def test_end_to_end_dry_run_approval_execute_uses_the_returned_approval_id(
    fake_kubectl, monkeypatch
):
    """The full real flow, with no hand-constructed ids anywhere:

    1. A real dry-run (`remediate.scale_deployment`) yields a real action_id.
    2. The real `sdk.request_approval_impl` (the exact code `_approval`'s MCP
       tool wrapper calls) parks on `ctx.request_approval`, which blocks on a
       genuine `hub` future — exactly like a live run.
    3. The parked approval is resolved exactly the way `POST /runs/:id/
       approve` resolves one: `db.decide_approval` then `hub.resolve_approval`
       (stubbed db, real hub) — the SAME sequence app.py's endpoint uses.
    4. The approval_id fed into `execute` is the one `request_approval_impl`
       RETURNED to the caller, not one the test peeked at some other way.

    This is the regression test for the CRITICAL fix: before it,
    `RunContext.request_approval` returned only the decision string, so the
    model (and this test) had no approval_id to hand back to the execute
    gate at all."""
    import asyncio

    from agent_service import context as context_module
    from agent_service.hub import hub
    from agent_service.tools import sdk

    store: dict[str, dict] = {}
    created_approval_ids: list[str] = []

    async def _add_approval(run_id, approval):
        store[approval.id] = {"run_id": run_id, "summary": approval.summary, "decision": None}
        created_approval_ids.append(approval.id)

    async def _get_approval(run_id, approval_id):
        row = store.get(approval_id)
        if row is None or row["run_id"] != run_id:
            return None
        return row

    async def _decide_approval(run_id, approval_id, decision):
        row = store.get(approval_id)
        if row is None or row["run_id"] != run_id or row["decision"] is not None:
            return False
        row["decision"] = decision
        return True

    async def _set_status(run_id, status, *, summary=None, ended=False):
        pass

    # remediate.db IS agent_service.db (same module object context.py's
    # `db.add_approval`/`db.set_status` calls resolve against) — patching it
    # here covers both RunContext's writes and _execute_gate's read.
    monkeypatch.setattr(remediate.db, "add_approval", _add_approval)
    monkeypatch.setattr(remediate.db, "get_approval", _get_approval)
    ...
    try:
        ...
        approval_task = asyncio.create_task(
            sdk.request_approval_impl(ctx, "scale gateway 2 -> 4", aid)
        )
        # Let the task run up to the point it parks on the hub future — none
        # of the stubbed db calls actually suspend, so one scheduling pass is
        # normally enough; poll briefly to avoid a flaky race regardless.
        for _ in range(200):
            if created_approval_ids:
                break
            await asyncio.sleep(0)
        assert created_approval_ids, "request_approval never persisted an approval row"
        pending_approval_id = created_approval_ids[-1]

        assert await _decide_approval(ctx.run_id, pending_approval_id, "approved")
        assert hub.resolve_approval(ctx.run_id, pending_approval_id, "approved")

        approval_result = await approval_task
        assert approval_result["decision"] == "approved"
        assert approval_result["approval_id"] == pending_approval_id

        executed = await remediate.scale_deployment(
            ctx, "gateway", replicas=4, dry_run=False,
