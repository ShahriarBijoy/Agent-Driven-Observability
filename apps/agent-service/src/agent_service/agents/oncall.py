...
def _build_prompt(
    ...
    return (
        ...
        "request_approval summary, execute once approved, then confirm recovery from the "
        "METRIC with mimir_query — check alert_status at most twice and never sit polling it: "
        "Alertmanager needs an evaluation cycle to flip, the closing step re-checks it "
        "server-side after your session ends, and turns spent waiting here are turns the "
        "postmortem does not get. Say so explicitly if the metric has not recovered. Then "
        "close with publish_postmortem — "
